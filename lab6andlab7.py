...
class Run:
    def __init__(self, factory):
        ...
        self.pidTheta = pid_controller.PIDController(500, 100, 0, [-10, 10], [-100, 100], is_angle=True)
        # self.pidDistance = pid_controller.PIDController(1000, 0, 50, [0, 0], [-200, 200], is_angle=False)
        self.pdWF = pid_controller.PIDController(200, 50, 0, [0,0], [-150, 150])
    ...
    def go_to_angle(self, goal_theta):
        while math.fabs(math.atan2(
            ...
            math.cos(goal_theta - self.odometry.theta))) > 0.2:
    ...
    def run(self):
        ...
        factor = 0.3
        waypoints = [
            [4.0 * factor, 7.0 * factor],
            [9.0 * factor, 3.0 * factor],
            [9.0 * factor, 9.0 * factor],
